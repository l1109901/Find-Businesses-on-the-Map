...
public class Randevu_Islemleri extends ActionBarActivity {
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        if (id == 999) {
            return new DatePickerDialog(Randevu_Islemleri.this, myDateListener, year, month, day);//<-----date icin dialog
        }
        if (id == 998) {
            return new TimePickerDialog(Randevu_Islemleri.this,myTimeListener,hour,min,true);//<----------time icin dialog
        }
        return null;
    }
    ...
    public void sendMessage(View view){
        String tarih=dateView.getText().toString();
        String saat=time.getText().toString();
        User user=userLocalStore.getLoggedInUser();
        long tcno=user.getTc_no();

        ServerRequests serverRequests=new ServerRequests(this);
        serverRequests.storeRandevuDataInBackground(tcno,tcno_firma,tarih,saat);
